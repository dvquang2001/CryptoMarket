...
fun CoinPricesChart(
    ...
    infos: List<PairData> = emptyList(),
    graphColor: Color = LightPrimary
) {
    val spacing = 200f
    ...
    val upperValue = remember(infos) {
        (infos.maxOfOrNull { it.price!! }?.plus(1))?.roundToInt() ?: 0
    }
    val lowerValue = remember(infos) {
        infos.minOfOrNull { it.price!! }?.toInt() ?: 0
    }
    ...
    Canvas(modifier = modifier) {
        ...
        (infos.indices step 2).forEach { i ->
            val info = infos[i]
            val hour = info.timeLong!!.convertToDateTime()
